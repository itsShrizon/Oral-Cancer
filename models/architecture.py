...
class MultiTaskOralClassifier(nn.Module):
    ...
    def __init__(self, backbone=None, num_subtypes=None, dropout=None, pretrained=None,
        ...
        # NOTE: backbone.num_features is unreliable for some timm models — e.g.
        # VGG with num_classes=0 still emits its pre_logits FC stack (4096),
        # but reports num_features=512 (the conv channels). Probe with a dummy
        # forward so the heads always match the real feature dim.
        with torch.no_grad():
            self.backbone.eval()
            probe = torch.zeros(1, 3, 224, 224)
            try:
                feat = self.backbone(probe)
                num_features = feat.shape[-1] if feat.dim() == 2 else feat.numel()
            except Exception:
