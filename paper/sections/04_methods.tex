...
The experimental data combine two complementary sources that together
support the dual-task formulation. \emph{Dataset~1} (DS1) provides binary
supervision: a curated collection of oral lesion photographs labelled as
benign or malignant, used to drive the binary head of the classifier.
\emph{Dataset~2} (DS2) provides seven-way subtype supervision across the
seven classes \textbf{CaS}, \textbf{CoS}, \textbf{Gum}, \textbf{MC},
\textbf{OC}, \textbf{OLP}, and \textbf{OT}, as labelled in the source
dataset; we retain the dataset's class abbreviations verbatim rather than
assigning clinical expansions that the source release does not document.
Within DS2, the subset $\{\text{MC},\text{OC},\text{CaS}\}$ is taken to
denote the malignant subtypes, and the remaining four classes the
non-malignant subtypes, for the purpose of constructing binary supervision
when DS2 images are read into the binary head. Representative images for the two binary categories
...
and Kolmogorov--Arnold Network attention (KAN-Attention) --- each preceded
